...
Sync intrinsics (fence / commit_group / wait_group) are schema-declared at the
top. The shape/dtype/transpose-parameterized mma_async family and descriptor
bitfield encoder are hand-written below.
"""

import tvm
from tvm.tirx.op import cuda_func_call

from .._schema import IntAttr, ptx_intrinsic
...
# Python API:
#     Tx.ptx.wgmma.fence()                 # wgmma.fence.sync.aligned
#     Tx.ptx.wgmma.commit_group()          # wgmma.commit_group.sync.aligned
#     Tx.ptx.wgmma.wait_group(n)           # wgmma.wait_group.sync.aligned <n>
# -----------------------------------------------------------------------------
ptx_intrinsic(
    op_name="ptx_wgmma_fence",
    ptx_template="wgmma.fence.sync.aligned;",
    helper_name="ptx_wgmma_fence",
)
ptx_intrinsic(
    op_name="ptx_wgmma_commit_group",
    ptx_template="wgmma.commit_group.sync.aligned;",
    helper_name="ptx_wgmma_commit_group",
)
ptx_intrinsic(
    op_name="ptx_wgmma_wait_group",
    attrs=[IntAttr("n")],
    ptx_template="wgmma.wait_group.sync.aligned {n};",
    helper_name_template="ptx_wgmma_wait_group_{n}",
)
