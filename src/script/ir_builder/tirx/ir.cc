...
Buffer BufferDecl(ffi::Array<PrimExpr> shape, DataType dtype, ffi::String buffer_name,
  ...
  if (!data.defined()) {
    ...
    buffer_data = tvm::tirx::Var(buffer_name,
...
Buffer BufferView(tvm::tirx::Buffer buffer, tvm::tirx::TLayout layout, Array<PrimExpr> shape) {
  ...
  if (auto tile_layout = layout.as<tvm::tirx::TileLayoutNode>()) {
    if (tile_layout->from.defined()) {
      ...
      ICHECK(tvm::tirx::ExecScope::Create(logical_scope).Is(tile_layout->from.value()))
...
SBlockFrame Block(ffi::String name, bool no_realize, ffi::String exec_scope,
  ...
  if (exec_scope.empty()) {
    ...
    n->exec_scope = tvm::tirx::ExecScope::Create(exec_scope);
  }
...
BlockFrame BlockFrameSlice(BlockFrame block, Optional<Array<Range>> slices,
  ...
  ICHECK(!block->exec_scope->IsInstance<tvm::tirx::ExecScopeSliceNode>())
      << "InternalError: Block frame already has an execution scope slice";
  block->exec_scope = tvm::tirx::ExecScopeSlice(slices, select_cond, block->exec_scope_slice_parent,
...
tvm::tirx::Var KernelId(PrimExpr extent) {
  ...
  TVM_FFI_ICHECK(frame->exec_scope->IsInstance<tvm::tirx::WorldScopeNode>())
      << "InternalError: exec_scope is not WorldScope.";
  tvm::tirx::Var scope_id("");
  frame->exec_scope = tvm::tirx::WorldScope(
      tvm::tirx::ScopeIdDef({scope_id}, {extent}, tvm::tirx::ScopePair("world", "kernel")));
  return scope_id;
}

ffi::Array<tvm::tirx::Var> KernelScopeId(ffi::Array<PrimExpr> extents, ffi::String parent,
                                        ffi::String name, ffi::String cur) {
  BlockFrame frame = FindBlockFrame(name);
  TVM_FFI_ICHECK(frame->exec_scope.defined()) << "InternalError: exec_scope is not defined.";
  TVM_FFI_ICHECK(frame->exec_scope->IsInstance<tvm::tirx::KernelScopeNode>())
...
Barrier AllocBarrier(ExecScope thread_scope, String name_hint) {
  Barrier barrier = tvm::tirx::Barrier(thread_scope, name_hint);
  IRBuilder builder = IRBuilder::Current();
...
BarrierArray AllocBarrierArray(ExecScope thread_scope, size_t size, String name_hint) {
  BarrierArray barrier_array = tvm::tirx::BarrierArray(thread_scope, size, name_hint);
  IRBuilder builder = IRBuilder::Current();
  if (Optional<BlockFrame> frame = builder->GetLastFrame<BlockFrame>()) {
...
TVM_STATIC_IR_FUNCTOR(Namer, vtable)
    .set_dispatch<tvm::tirx::BarrierNode>([](const ObjectRef& node, ffi::String name) -> void {

    });

TVM_STATIC_IR_FUNCTOR(Namer, vtable)
    .set_dispatch<tvm::tirx::BarrierArrayNode>([](const ObjectRef& node, ffi::String name) -> void {
